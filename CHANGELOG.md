...
## 4.5.0
- Lower the supported `analyzer` floor from 10.0.0 to 8.2.0, so DCL can be added to projects that pin an older analyzer. 8.2.0 is the lowest reachable version without widening the `analysis_server_plugin: ^0.3.0` constraint, because plugin 0.3.0 exact-pins analyzer 8.2.0. The modern Analysis Server plugin keeps working across the whole range. Note that every analyzer 8.2.0+ requires Dart SDK `^3.9.0`, so this widens analyzer compatibility without lowering the effective SDK floor. One consequence lands on the test corpus rather than on users: the compatibility matrix runs the whole corpus on every supported analyzer, so a fixture written in syntax an older analyzer cannot parse fails the low rows even when `lib/` analyzes clean there. Primary constructors, first parsed by analyzer 9.0.0, already did this to the `prefer-static-class` correct example. Where it happens the fixture is kept and the test probes the capability at run time, asserting the degraded behaviour on analyzers that lack it rather than skipping, so the guard fails loudly once it stops being needed.
...
- Fix type names resolving to nothing when the declaration carries a documentation comment or an annotation on analyzer 10.0.0+. A declaration lists its documentation and its metadata ahead of its own children, so a reading that took the first child node landed on the comment or on the first annotation, neither of which carries an identifier token of its own. Released versions are affected for extension types, where a documented or annotated one reported an empty scope name in metrics output and in the qualified names of the functions it declares. The equivalent class and enum readings were introduced and fixed inside this release.
