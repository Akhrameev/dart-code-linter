...
## 4.4.2
- Fix `no-equal-arguments` reporting for three or more equal arguments. The rule looked up the last equal argument for each argument in turn, so a call like `fn(value, value, value)` produced one report per earlier occurrence, every one of them pointing at the same last argument, while the argument in between was never reported. Each argument that repeats an earlier one is now reported once, at its own location. Calls with at most two equal arguments, and calls whose duplicates are interleaved, are reported exactly as before.
