...
## 4.4.1
- Fix the name reported for an unnamed constructor by `check-unnecessary-nullable`. Analyzer 14.4.0 dropped the special case in `ConstructorElement.displayName` that returned the bare type name for the unnamed constructor, so that declaration was reported as `Type.new` there and as `Type` on every earlier analyzer. The name is now normalized in this package, so the report no longer depends on which analyzer the consumer resolved. Named constructors are unaffected and keep their `Type.name` form.
- Widen the `pub_updater` constraint to `>=0.3.0 <0.7.0`. The previous ceiling excluded pub_updater 0.6.0, which requires Dart 3.12.0; on that SDK the package lost the pub points for supporting up-to-date dependencies.
